...
namespace Capstone
{
    public class Inventory
    {
        ...
        public decimal Dispense(string input, decimal money, int quant)
        {
            VendingInventory[input].Quantity -= quant;
            VendingInventory[input].TotalSold++;
            money = money - VendingInventory[input].Price;
            Console.Write("");
            VendingInventory[input].DispenseMessage();
        ...
        public void SelectProduct (string userSelect, decimal money, string path)
        {
            if (VendingInventory.ContainsKey(userSelect))
            {
                if (VendingInventory[userSelect].Quantity > 0)
                {
                    if (VendingInventory[userSelect].Price <= money)
                    {
                        money = Dispense(userSelect, money);
                        File.AppendAllText(path, $"\n{DateTime.Today.Month}/{DateTime.Today.Day}/{DateTime.Today.Year}  {DateTime.Now.Hour}:{DateTime.Now.Minute}:{DateTime.Now.Second} {userSelect} {VendingInventory[userSelect].SnackName} {money:C2}");
                    }
                    else
                    {
        ...
        public void SelectProduct(string userSelect, decimal money, string path, int purchaseQuantity)
        {
            if (VendingInventory.ContainsKey(userSelect))
            {
                if (VendingInventory[userSelect].Quantity > 0)
                {
                    if(VendingInventory[userSelect].Quantity > purchaseQuantity)
                    {
                        if (VendingInventory[userSelect].Price <= money)
                        {
                            money = Dispense(userSelect, money, purchaseQuantity);
                            File.AppendAllText(path, $"\n{DateTime.Today.Month}/{DateTime.Today.Day}/{DateTime.Today.Year}  {DateTime.Now.Hour}:{DateTime.Now.Minute}:{DateTime.Now.Second} {userSelect} {VendingInventory[userSelect].SnackName} {money:C2}");
                        }
                        else
                        {
                    ...
                    {
                        Console.WriteLine("Not enough product for selected quantity");
                    }   
                }
                else
                {
